...
\subsection{Smearing behaviour of distillation}
    To compute physical properties of mesonic states they have to be implemented on a lattice which lattice spacing corresponds with the size of a meson. Using local meson creation operators like \mref{meson_creation_operator} are not adequate because they only create a meson on a fixed lattice poinst. Hence it is crucial to apply smearing to the states. Similar to Gaussian smearing, the distilled quark is of gaussian shape.\\
    
    Expanding a local meson state $\ket{\psi}$ in a basis of eigenvectors of the lattice laplacian we arrive at:
    \begin{equation}
        \begin{aligned}
        ...
        \end{aligned}
    \end{equation}
    If we restrict the eigenmodes to some number N, this equation becomes the completeness relation becomes the distillation operator \mref{distillation_operator}. Hence the more eigenmodes we remove, the larger the width of the wave $\psi(\textbf{x})$ becomes. If we remoce too many eigenmodes the simulation will get distorted.
